...
public class WalkService {
  ...
  */
  ...
  public WalkResponseDto walkOver(
      WalkRequestDto walkRequestDto,
      String key,
      ConcurrentHashMap<String, List<PingRequestDto>> pingList,
      Long petId) {
    // Activity 저장
  ...
  public List<WalkResponseDto> findAllByPetId(Long petId) {
    ...
    for (Walk w : walkList) {
      WalkResponseDto wrd =
          new WalkResponseDto(
              w.getStartPoint(), w.getEndPoint(), w.getRoadMap(), w.getActivity(), w.getWalkDate());
      wrdList.add(wrd);
    }
  ...
  public RoadMap findRoadMapById(Long walkId) {
    return walkRepository.findById(walkId).get().getRoadMap();
  }
  ...
  /** 핫 스팟 좌표리스트를 반환해주는 함수 (hotplace랑 같은 기능) 근데 이건 머문 시간 계산 */
  ...
  /**
   * 모든 산책리스트에서 산책로안에있는 pingList만 뽑아서 해당되는 pet의 petId와 연결하여 HashMap에 저장 (key: petId, value:
   * List<Ping>)
   */
  private void setPingMap(List<Walk> allWalkList, HashMap<String, List<Coord>> coordMap) {
    ...
    for (Walk w : allWalkList) {
      // 에뮬레이터 데이터는 빼버림1
      if (w.getStartPoint().getId() == w.getEndPoint().getId()) continue;
      ...
      for (int i = 0; i < pingList.size(); i++) {
        ...
        // 에뮬레이터 데이터 거르기2
        if (curPing.getAltitude() == Integer.valueOf(5).doubleValue()) continue;
  ...
  public Long getCalcStayTime(Date preDate, Date curDate) {
    ...
    LocalDateTime preLocalDateTime1 =
        preDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
    LocalDateTime curLocalDateTime2 =
  ...
  /**
   * 공간 좌표를 좌표 평면상에 옮기면서 가장 큰 좌표와 가장 작은 좌표를 기준으로 생기는 사각형의 4 꼭짓점을 반환해준다. 단, 이때 가장 작은 좌표는 소수점 둘째 자리에서
   * 내림 (ex. 37.200000) 가장 큰 좌표는 소수점 둘째 짜리에서 반올림 (ex. 37.500000)
   */
  ...
  public Board makeBoard(HashMap<String, Object> rectVertex, double areaN, int areaN_int) {
    ...
    int boardSize =
        Integer.parseInt(rectVertex.get("rectArea").toString())
            * (int) Math.pow(areaN_int, 2); // 바둑판에 있는 공간의 총 개수

    return new Board(boardSize, width, height, areaN, new HashMap<>());
  }

  /** 모든 핑을 보드의 각 스팟에 분류함 */
  public void classifyAllPings(
      Board board,
      HashMap<String, Object> rectVertex,
      HashMap<String, List<Coord>> coordMap,
      double areaN,
      int areaN_int) {
    Collection<List<Coord>> list_coordList =
        coordMap.values(); // coordMap에서 모든 value값만 빼옴(즉, coordList 들만 빼옴)
    ...
    for (Coord c : allCoordList) {
      ...
      if (!spotList.containsKey(Integer.toString(spot_index))) { // 보드판에서 해당 spot이 빈 스팟일때
        // 스팟 생성
        double spot_min_lat_range =
            DoubleCalc.div(Math.floor(c.getLatitude() * areaN_int), (double) areaN_int, 3);
        double spot_max_lat_range =
            DoubleCalc.div(Math.ceil(c.getLatitude() * areaN_int), (double) areaN_int, 3);
        double spot_min_lng_range =
            DoubleCalc.div(Math.floor(c.getLongitude() * areaN_int), (double) areaN_int, 3);
        double spot_max_lng_range =
            DoubleCalc.div(Math.ceil(c.getLongitude() * areaN_int), (double) areaN_int, 3);
  ...
  /** 모든 좌표를 베이스로 하되 각 좌표별 머문 시간을 확인하여 머문 시간이 일정 시간 이상인 좌표로만 핫스팟을 판별 */
  private List<Spot> findHotSpotByStayTime(Board board) {
    HashMap<String, Spot> sl = board.getSpotList(); // 보드에서 spot 리스트를 뺴옴
    List<Spot> hotSpotList = new ArrayList<>(); // 핫스팟만 저장할 스팟 리스트
    int stayTime_condition = 4; // 머문시간 조건, 한 좌표에 머문 시간이 해당 값 이상이어야함
    int hotSpot_condition =
        5; // 핫스팟이 되기 위한 조건, 해당 스팟에 stayTime_condition을 만족시키는 좌표의 개수가 해당 값 이상이어야함
